...
const idVendedor = params.get("id"); // Carga vendedor no el idinmueble 
...
async function cargarComentarios(page = 0, size = currentSize){
  try{
    if(!idVendedor) idVendedor = await obtenerIdUsuarioInmueble(idInmueble);
...
console.log(idVendedor)
    ...
    document.querySelectorAll(".borrar-btn").forEach(btn => btn.addEventListener("click", () => abrirConfirmEliminar(btn.dataset.id)));
    document.querySelectorAll(".editar-btn").forEach(btn => btn.addEventListener("click", () => manejarEditar(btn.dataset.id, btn.dataset.comentario, btn.dataset.puntuacion)));
...
if(form){
  form.addEventListener("submit", async(e) => {
    e.preventDefault();
    if(!idUsuarioActual || !idVendedor){ mostrarError("No se encontró usuario o vendedor."); return; }
