...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    cartaContainer.innerHTML = `
      <section class="card" id="card${casa.idinmuebles}">
        <img src="${foto}" alt="${casa.titulo}" />
        <div class="info">
          <div class="star-favorite" onclick="event.stopPropagation()">
    ...
    // Obtenemos el checkbox recién creado
    const checkbox = document.getElementById(`fav${casa.idinmuebles}`);

    // Inicializamos favoritos del usuario
    await inicializarFavoritos(usuario.idusuario);
    ...
    async function inicializarFavoritos(usuarioId) {
      const favoritos = await obtenerFavoritos(usuarioId);
      favoritos.forEach(fav => {
        const checkbox = document.getElementById(`fav${fav.idInmueble}`);
        if (checkbox) {
    ...
    // Detener propagación en la estrella
